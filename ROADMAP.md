...
pure geometry had nine tests here in `tests/geo.test.js` (82 assertions total at the time), including
the one that matters most: `routeProgress` searching forward so an out-and-back route does not snap
the return leg onto the outbound one and mark the wrong half. The suite has grown since — see "Test
coverage is thin" below for the current total, which is well past 82.
...
- **Test coverage is thin.** `tests/geo.test.js` now covers the pure maths in `safewalk-app/geo.js`:
  distances, street graph and shortest paths, polyline decoding, rating bands, colour contrast,
  bearing-to-words, PostGIS hex EWKB parsing (where a police warning's location comes from), the
  geo.js/app.js name-shadowing guard, `isOpenNow`'s refuge-hours logic, route scoring, hazard-weighted
  route scoring against police and incident reports, and where a mark made while walking lands. Run
  it with `node tests/geo.test.js`.
  This entry has said "82 assertions" for a while — actually running the suite in this environment
